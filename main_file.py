...
def start_message(message): # Функия подписки на рассылку
    ...
    if message.chat.id == 171776816: #Бесконечный цикл обновления и вывода новостей
        t_now = int(time.time())
        while True:
...
def start_message(message): # Вывод информации о подписчиках для администратора
    if message.chat.id == 171776816:
        for id in Parser.find_user_id():
            bot.send_message('171776816', str(id))

@bot.message_handler(commands=['stop'])
def start_message(message): #Функция отписки от новостей
